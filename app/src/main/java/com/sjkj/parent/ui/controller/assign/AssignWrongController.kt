...
import com.sjkj.parent.ui.fragment.assign.AssignDetailWrongFragment
 ...
 */
class AssignWrongController(context: Context) : BaseRecycleController<Library>(context), AssignContract.BaseView {
    ...
    private var startDate: String? = getMonthStartDate()

    private var endDate: String? = getMonthToday()

    fun setCourseInfoID(courseInfoID: Int) {
        this.courseInfoId = courseInfoID
        if (baseList.size == 0)
            assignWrongPresenter?.getWrongData(courseInfoID)
    }
    ...
    override fun initData() {
        ...
        timeView.setOnTimeCheckListener(object : TimeView.OnTimeCheckListener {
            override fun confrim(startDate: String, endDate: String) {
                this@AssignWrongController.startDate = startDate
                this@AssignWrongController.endDate = endDate
            }
        })
        nodeAdapter?.addHeaderView(timeView)
    ...
    fun preview() {
        ...
        val fragment = AssignDetailWrongFragment().newInstance(getKnowIDs(), courseInfoId, startDate, endDate)
        startFragment(fragment)
    }

    private fun getKnowIDs(): String {
