...
import com.sjkj.parent.ui.controller.assign.AssignWrongController
 ...
 */
class AssignWrongFragment : BackBaseFragment() {
    ...
    private fun initData() {
        ...
        mTopBar?.addRightTextButton(R.string.preview, R.id.topbar_right_about_button)
                ?.onClick {
                    ...
                    if (view != null)
                        (view as AssignWrongController).preview()
                }
        courseInfoList = getCourseInfoList()
        courseInfoList?.forEachIndexed { index, bean ->
            if (index == 0)
                courseInfoID = bean.CourseInfoID
            with(bean) {
    ...
    private fun initPage() {
        ...
        mContentViewPager?.addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
            ...
            override fun onPageSelected(position: Int) {
                ...
                val view = mPageMap.get(courseInfoID)
                if (view != null)
                    (view as AssignWrongController).setCourseInfoID(courseInfoID)
    ...
    private fun getPageView(courseInfoID: Int, position: Int): View {
        ...
        if (view == null) {
            view = AssignWrongController(context)
            if (position == 0)
                view.setCourseInfoID(courseInfoID)
            view.setHomeControlListener(object : AssignWrongController.HomeWorkControlListener {
