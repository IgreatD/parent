...
public class TreeHelper {
    ...
    public static <T> List<Node> getSortedNodes(List<T> datas,
                                                int defaultExpandLevel, boolean canClick) throws IllegalArgumentException,
            IllegalAccessException

    {
        List<Node> result = new ArrayList<Node>();
        // 将用户数据转化为List<Node>
        List<Node> nodes = convetData2Node(datas, canClick);
        // 拿到根节点
     ...
     * @throws IllegalArgumentException
     */
    private static <T> List<Node> convetData2Node(List<T> datas, boolean canClick)
...

    {
        ...
        for (T t : datas) {
            ...
            for (Field f : declaredFields) {
                ...
                if (canClick) {
                    if (id != -1 && pId != -1 && label != null && click != -2) {
                        break;
                    }
            ...
            node = new Node(id, pId, label, click, num);
            nodes.add(node);
        }
